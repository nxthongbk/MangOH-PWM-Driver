...
char pwm_driver_i2c_bus[256] = "/dev/i2c-1";

/**
 * Function: Configure I2C hub to enable I2C bus that connected to 
...
{
	int result;
	int i2c_fd = open(pwm_driver_i2c_bus, O_RDWR);
	if (i2c_fd < 0) {
...
void set_pwm(uint8_t pin, uint16_t led_on, uint16_t led_off)
{
	...
	res = I2C_write_bytes(PWM_DEFAULT_I2C_ADDR, PWM_LED0_ON_L+4*pin, 4, buffer);
	if (res < 0)
	{
		LE_ERROR("write bytes failed with error %d\n", res);
	}
}
